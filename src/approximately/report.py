...
def _neighbours_card(store, trace: Trace) -> str:
    ...
    try:
        ...
        ranked = rank_similar(trace, store.list_traces(),
                              top=NEIGHBOUR_TOP)
    except Exception:
        return ""
    if not ranked:
    ...
    rows = "".join(
        f"<tr><td class=\"k\">{_html.escape(c.id)}</td>"
        f"<td>{r:.2f}</td>"
        f"<td>{_html.escape((c.task or '')[:60])}</td></tr>"
        for c, r in ranked)
    return ('<div class="card"><h2>Nearest neighbours</h2>'
            '<p style="margin:4px 0 10px;color:#6c757d;font-size:13px">'
            'Runs in this store with the most similar shape:</p>'
            '<table class="steps"><tr><th>trace</th><th>similarity</th>'
            '<th>task</th></tr>' + rows + "</table></div>")
...
def render_html(trace: Trace, report: FailureReport,
    ...
    neighbours_card = _neighbours_card(store, trace)
